...
const UserSubjectsModal = ({ handleCloseUserSubject, professorsSubjects, users, professors, subjects, subjectsOfStudent }) => {
  const {
    ...
    try {
      for (let k = 0; k < subjectsOfStudent.length; k++) {
        ...
        for(let i = 0; i < studentSubjects.length; i++) {
          ...
          if (studentSubjects[i].professorId === subjectsOfStudent[k].professorId && studentSubjects[i].subjectId === subjectsOfStudent[k].subjectId) {
            found = true;
            break;
          }
        }
        ...
        if(subjectsOfStudent[k].selected === true && !found) {
            contract.methods.addStudentSubjects(users.user.id,subjectsOfStudent[k].professorId,subjectsOfStudent[k].subjectId).send({ from: accounts[0] });
        } 
        if (subjectsOfStudent[k].selected === false && found) {
              contract.methods.deleteStudentSubjects(users.user.id,subjectsOfStudent[k].professorId,subjectsOfStudent[k].subjectId).send({ from: accounts[0] });
        }
    ...
    >
      ...
      >
        ...
        <Box display='flex' flexDirection='column' my={1}>
          <Typography variant='h4'>Enrollment of Student {users.user.name}</Typography>
          <Box my={2}>
                          ...
                          <TableBody>
                            {professorsSubjects.map((ps) => {
                              ...
                              />
                                ...
                                <TableCell component="th" scope="row">
                                  {professors.map((prof)=> {if(prof.id === ps.professorId) return  prof.name})}
                                </TableCell>
                                <TableCell component="th" scope="row">
                                  {subjects.map((sub)=> {if(sub.id === ps.subjectId) return  sub.name})}
                                </TableCell>
                              </TableRow>)
                            })}
                          </TableBody>
